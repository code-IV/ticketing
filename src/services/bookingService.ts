...
import { Booking, BookingItem, Ticket, PaginatedResponse, ApiResponse } from '@/types';
...
export const bookingService = {
  ...
  async getMyBookings(page = 1, limit = 20): Promise<PaginatedResponse<Booking>> {
  ...
  async getBookingByReference(reference: string): Promise<ApiResponse<{ booking: Booking }>> {
  ...
  async getBookingTickets(id: string): Promise<ApiResponse<{ tickets: Ticket[] }>> {
