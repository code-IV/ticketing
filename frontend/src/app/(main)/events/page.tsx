            ...
            return (
              ...
              >
                ...
                <div className={`flex-1 p-6 lg:p-10 flex flex-col ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>
                  <h2 className="text-2xl md:text-4xl font-black tracking-tighter mb-4 leading-tight">{event.name}</h2>
                  <p className={`mb-2 text-sm md:text-base leading-relaxed line-clamp-3 ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>{event.description}</p>
                  ...
                  <div className="mt-auto flex items-center justify-between gap-5 pt-6  border-t border-gray-100 dark:border-gray-800">
                    ...
                    <Link href={`/events/${event.id}`}>
                      <button disabled={soldOut} className={`px-5 py-4 rounded-3xl font-black text-xs uppercase tracking-widest transition-all ${soldOut ? 'bg-gray-200 text-gray-400' : 'bg-accent2 text-white hover:bg-accent hover:text-black shadow-lg'}`}>
      ...
      {totalPages > 1 && (
        <div className="max-w-7xl mx-auto px-4 mt-16 flex flex-col items-center gap-6">
          <div className="flex items-center gap-4">
            <button onClick={handlePreviousPage} disabled={page === 1} className="p-4 rounded-2xl bg-[#ffd84f] text-black disabled:opacity-30 transition-all hover:scale-110">
              <ChevronLeft size={20} />
            </button>
            <span className={`font-black text-lg ${isDarkTheme ? 'text-white' : 'text-black'}`}>
              {page} <span className="text-[#ffd84f]">/</span> {totalPages}
            </span>
            <button onClick={handleNextPage} disabled={page === totalPages} className="p-4 rounded-2xl bg-[#ffd84f] text-black disabled:opacity-30 transition-all hover:scale-110">
              <ChevronRight size={20} />
            </button>
          </div>
          <div className="flex gap-2">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              <div key={n} className={`h-1 rounded-full transition-all ${page === n ? 'w-8 bg-[#ffd84f]' : 'w-2 bg-gray-300'}`} />
            ))}
          </div>
        </div>
      )}
