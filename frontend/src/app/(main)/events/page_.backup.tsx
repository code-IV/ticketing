...
export default function EventsPage() {
  ...
  const getBannerImages = (event: Event) => {
    ...
    return banners.length > 0
      ? banners
      : [{ url: "/placeHolder.jpg", type: "image", label: "placeholder" }];
  };
  ...
  useEffect(() => {
    const startCycling = (eventId: string) => {
      if (intervalRefs.current[eventId])
        clearInterval(intervalRefs.current[eventId]);
            ...
            return (
              ...
              >
                ...
                <div className="relative h-64 lg:h-auto lg:w-[450px] shrink-0 overflow-hidden bg-[#ffd84f]">
                  <img
                    src={getCurrentBanner(event)?.url}
                    alt={event.name}
                    className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  />
                  <div className="absolute top-4 left-4">
                    <div
                      className={`${isDarkTheme ? "bg-black/90" : "bg-white/90"} p-3 rounded-2xl border border-white/10`}
                    >
                      <p className="text-[10px] font-black text-[#ffd84f] uppercase tracking-widest">
                        Tickets Left
                      </p>
                      <p
                        className={`text-xl font-black ${isDarkTheme ? "text-white" : "text-slate-900"}`}
                      >
                ...
                >
                  ...
                  <p
                    className={`mb-6 text-sm ${isDarkTheme ? "text-gray-300" : "text-gray-600"}`}
                  >
                    {event.description ||
                  ...
                  <div className="mt-auto flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-black text-[#ffd84f] uppercase block">
                        Price
                      </span>
                    ...
                    <Link
                      ...
                      className={`px-8 py-4 rounded-full font-black text-sm uppercase tracking-wider transition-all ${
                        soldOut
                          ...
                          : "bg-[#ffd84f] text-black hover:scale-105"
                      }`}
                    >
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-12">
          <button
            ...
            disabled={page === 1}
            className="p-3 bg-[#1a1a1a] rounded-xl text-white disabled:opacity-50"
          >
            <ChevronLeft />
          </button>
          <span className="font-black text-[#ffd84f]">
            {page} / {totalPages}
          </span>
          <button
