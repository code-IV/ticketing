...
const getTicketTypeData = (gameId: number): TicketTypeData[] => {
  if (gameId === 1) {
    return [
      { type: 'Adult Premium', category: 'Adult', sold: 600, revenue: 51000, avgPrice: 85, performance: 85 },
      { type: 'Child Basic', category: 'Child', sold: 450, revenue: 13500, avgPrice: 30, performance: 75 },
      { type: 'Adult Standard', category: 'Adult', sold: 350, revenue: 17500, avgPrice: 50, performance: 70 },
      { type: 'Senior Discount', category: 'Senior', sold: 120, revenue: 6000, avgPrice: 50, performance: 65 },
      { type: 'Student Package', category: 'Student', sold: 280, revenue: 14000, avgPrice: 50, performance: 60 },
      { type: 'Group Package', category: 'Group', sold: 300, revenue: 18000, avgPrice: 60, performance: 80 },
    ];
  } else if (gameId === 2) {
    return [
      { type: 'Adult Racing', category: 'Adult', sold: 320, revenue: 20800, avgPrice: 65, performance: 78 },
      { type: 'Child Mini', category: 'Child', sold: 280, revenue: 11200, avgPrice: 40, performance: 72 },
      { type: 'Adult Standard', category: 'Adult', sold: 200, revenue: 10000, avgPrice: 50, performance: 68 },
      { type: 'Senior Special', category: 'Senior', sold: 80, revenue: 4000, avgPrice: 50, performance: 62 },
      { type: 'Student Basic', category: 'Student', sold: 150, revenue: 6000, avgPrice: 40, performance: 58 },
      { type: 'Family Bundle', category: 'Group', sold: 70, revenue: 4200, avgPrice: 60, performance: 75 },
    ];
  } else if (gameId === 3) {
    return [
      { type: 'Adult VIP', category: 'Adult', sold: 800, revenue: 120000, avgPrice: 150, performance: 95 },
      { type: 'Child Adventure', category: 'Child', sold: 500, revenue: 25000, avgPrice: 50, performance: 85 },
      { type: 'Adult Premium', category: 'Adult', sold: 600, revenue: 48000, avgPrice: 80, performance: 88 },
      { type: 'Senior Explorer', category: 'Senior', sold: 200, revenue: 12000, avgPrice: 60, performance: 75 },
      { type: 'Student Quest', category: 'Student', sold: 300, revenue: 18000, avgPrice: 60, performance: 70 },
      { type: 'Group Quest', category: 'Group', sold: 400, revenue: 20000, avgPrice: 50, performance: 82 },
    ];
  }
  // Fallback data for unknown games
  return [
    { type: 'Standard Ticket', category: 'Adult', sold: 100, revenue: 5000, avgPrice: 50, performance: 50 },
    { type: 'Child Ticket', category: 'Child', sold: 50, revenue: 1500, avgPrice: 30, performance: 40 },
    { type: 'Senior Ticket', category: 'Senior', sold: 30, revenue: 900, avgPrice: 30, performance: 35 },
    { type: 'Group Package', category: 'Group', sold: 20, revenue: 1000, avgPrice: 50, performance: 45 },
  ];
};
...
const getTicketPerformanceData = (gameId: number): TicketPerformanceData[] => {
  ...
  return ticketData.map(ticket => ({
    ...
    performanceScore: Math.round((ticket.revenue / ticket.sold) * (ticket.sold / 100)),
    popularity: Math.round((ticket.sold / ticketData.reduce((sum, t) => sum + t.sold, 0)) * 100),
  }));
};

const getRevenueContributionSeries = (gameId: number, days = 30): RevenueContributionData[] => {
  return Array.from({ length: days }, (_, i) => {
...
const getRevenuePerTicketSeries = (gameId: number, days = 30): RevenuePerTicketData[] => {
  return Array.from({ length: days }, (_, i) => {
    const date = subDays(new Date(), days - 1 - i);
...
const COLORS = ['#3b82f6', '#f97316', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#ec4899'];
      ...
      <div className="max-w-7xl mx-auto">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <KpiCard title="Total Bookings" value={selectedGame.totalBookings} icon={Ticket} />
            <KpiCard title="Avg Occupancy" value={`${selectedGame.avgOccupancy}%`} icon={PieChart} />
            <KpiCard title="Events" value={selectedGame.eventsCount} icon={Calendar} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Top Performing Ticket Types</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={ticketTypeData}>
                  ...
                  <Tooltip 
                    formatter={(value: any, name: string) => [
                      name === 'sold' ? `${value} tickets` :
                      name === 'revenue' ? `$${value}` :
                      name === 'avgPrice' ? `$${value}` :
                      value,
                      name === 'sold' ? 'Tickets Sold' :
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Revenue Per Ticket Type (Last 30 days)</h3>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={revenuePerTicketSeries}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="vipRevenue" stroke="#8b5cf6" name="VIP Revenue" />
                  <Line type="monotone" dataKey="premiumRevenue" stroke="#f97316" name="Premium Revenue" />
                  <Line type="monotone" dataKey="standardRevenue" stroke="#10b981" name="Standard Revenue" />
                  <Line type="monotone" dataKey="groupRevenue" stroke="#f59e0b" name="Group Revenue" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Ticket Type Distribution</h3>
              <ResponsiveContainer width="100%" height={300}>
                <RePieChart>
                  <Pie
                    data={ticketTypeData}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Best Performing Ticket Types</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={ticketPerformanceData} layout="vertical">
                  ...
                  <Tooltip 
                    formatter={(value: any, name: string) => [
                      name === 'ticketsSold' ? `${value} tickets` :
                      name === 'revenue' ? `$${value}` :
                      name === 'performanceScore' ? `${value} pts` :
                      value,
                      name === 'ticketsSold' ? 'Tickets Sold' :
                      name === 'revenue' ? 'Revenue' :
                      name === 'performanceScore' ? 'Performance Score' :
                      name
                    ]}
                    labelFormatter={(label) => {
                      const ticket = ticketPerformanceData.find(t => t.type === label);
                      if (ticket) {
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <h3 className="font-semibold text-gray-900 mb-4">Revenue Contribution by Ticket Type</h3>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={revenueContributionSeries}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="adultRevenue" stroke="#3b82f6" name="Adult Revenue" strokeWidth={2} />
                  <Line type="monotone" dataKey="childRevenue" stroke="#10b981" name="Child Revenue" strokeWidth={2} />
                  <Line type="monotone" dataKey="seniorRevenue" stroke="#f59e0b" name="Senior Revenue" strokeWidth={2} />
                  <Line type="monotone" dataKey="studentRevenue" stroke="#8b5cf6" name="Student Revenue" strokeWidth={2} />
                  <Line type="monotone" dataKey="groupRevenue" stroke="#ef4444" name="Group Revenue" strokeWidth={2} />
                  <Line type="monotone" dataKey="promotionalRevenue" stroke="#ec4899" name="Promotional Revenue" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200">
              <h3 className="font-semibold text-gray-900">Events for {selectedGame.name}</h3>
            </div>
              ...
              <tbody>
                {gameEvents.map(event => (
                  <tr key={event.id} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-2">{event.ticketsSold}/{event.capacity}</td>
                    <td className="px-4 py-2">
