...
export default function EventDetailPage() {
  ...
  const eventName = `Event ${eventId.substring(0, 8)}...`; // temporary
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            <div className="flex justify-center items-center py-12">
              <Loader2 className={`w-8 h-8 animate-spin ${isDarkTheme ? "text-white" : "text-gray-900"}`} />
            </div>
          ) : (
            <>
              {/* KPI Cards */}
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <RePieChart>
                      <Pie
                        ...
                        dataKey="revenue"
                      >
                        {analytics.revenueByTicketType.map((entry: any, index: number) => (
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <RePieChart>
                      <Pie
                        ...
                        dataKey="sold"
                      >
                        {analytics.bookingsByTicketType.map((entry: any, index: number) => (
              ...
              <div className={`rounded-xl overflow-hidden ${isDarkTheme ? "bg-[#0A0A0A] border-gray-700" : "bg-white border-gray-200"}`}>
                ...
                <table className="w-full text-sm">
                  <thead className={isDarkTheme ? "bg-[#1a1a1a]" : "bg-gray-50"}>
                    <tr>
                      <th className={`px-4 py-2 text-left ${isDarkTheme ? "text-gray-400" : "text-gray-700"}`}>Type</th>
                      <th className={`px-4 py-2 text-left ${isDarkTheme ? "text-gray-400" : "text-gray-700"}`}>Price</th>
                      <th className={`px-4 py-2 text-left ${isDarkTheme ? "text-gray-400" : "text-gray-700"}`}>Sold</th>
                  ...
                  <tbody>
                    {analytics.ticketTypesTable.map((ticket, index) => (
                      <tr key={ticket.id || index} className={`border-t ${isDarkTheme ? "border-gray-700" : "border-gray-100"}`}>
                        <td className={`px-4 py-2 font-medium ${isDarkTheme ? "text-white" : ""}`}>{ticket.type}</td>
                        <td className={`px-4 py-2 ${isDarkTheme ? "text-gray-300" : ""}`}>${parseFloat(ticket.avgPrice).toFixed(2)}</td>
                        <td className={`px-4 py-2 ${isDarkTheme ? "text-gray-300" : ""}`}>{ticket.sold}</td>
                        <td className={`px-4 py-2 ${isDarkTheme ? "text-gray-300" : ""}`}>${parseFloat(ticket.revenue).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
