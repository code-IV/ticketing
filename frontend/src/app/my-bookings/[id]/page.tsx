...
const QRModal = ({ isOpen, onClose, guestName, refId, qrValue, isDarkTheme }: { isOpen: boolean; onClose: () => void; guestName: string; refId: string; qrValue: string; isDarkTheme: boolean }) => (
  <AnimatePresence>
    {isOpen && (
      <motion.div
        initial={{ opacity: 0 }}
        ...
        >
          ...
          <div className={`p-10 flex flex-col items-center ${isDarkTheme ? 'bg-bg3' : 'bg-gray-50'}`}>
            ...
            <div className="grid grid-cols-2 gap-4 w-full">
              ...
              <button className={`flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest ${isDarkTheme ? 'bg-gray-700 text-white' : 'bg-slate-100 text-slate-900'}`}>
...
const CollectorTicketCard = ({ item, index, isDarkTheme }: { item: { id: string; name: string; purchased: number; used: number; image?: string }; index: number; isDarkTheme: boolean }) => {
  ...
  const ticketGroups =
    booking.items?.map((item) => {
      const typeTickets = tickets.filter(
        (t) => t.ticket_type_id === item.ticket_type_id,
      );
      return {
        id: item.id,
    ...
    <div className={`min-h-screen ${isDarkTheme ? 'bg-[#0A0A0A]' : 'bg-[#F8FAFC]'} pb-24 pt-10 px-4`}>
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-14 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            >
              ...
              <div className={`relative bg-bg1 p-5 rounded-[32px] border border-slate-100 shadow-sm flex flex-col items-center justify-center gap-1.5 min-w-[100px] ${isDarkTheme ? 'bg-[#1a1a1a] border-gray-700 text-white' : 'bg-white border-slate-100 text-slate-900'}`}>
                <QrCode size={36} className={`${isDarkTheme ? 'text-white' : 'text-slate-900'}`} />
          ...
          <div className={`hidden sm:flex items-center gap-6 p-6 rounded-[40px] shadow-sm ${isDarkTheme ? 'bg-bg3 border-gray-700' : 'bg-white border-slate-100'}`}>
            <div className="text-right">
              <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isDarkTheme ? 'text-gray-400' : 'text-slate-400'}`}>
                Status
              </p>
              <span
            ...
            <div className={`w-px h-10 ${isDarkTheme ? 'bg-gray-700' : 'bg-slate-100'}`} />
            <div className="text-right">
              <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isDarkTheme ? 'text-gray-400' : 'text-slate-400'}`}>
                Amount
              </p>
              <p className={`text-xl font-black tracking-tighter ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          <AnimatePresence>
            {ticketGroups.map((item, i) => (
              <CollectorTicketCard key={item.id} item={item} index={i} isDarkTheme={isDarkTheme} />
            ))}
          </AnimatePresence>
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className={`lg:col-span-2 rounded-[48px] p-10 shadow-sm ${isDarkTheme ? 'bg-bg3 border-accent border' : 'bg-white border border-slate-100'}`}>
            <h3 className={`text-xs font-black uppercase tracking-[0.2em] mb-8 ${isDarkTheme ? 'text-gray-400' : 'text-slate-400'}`}>
              Event Itinerary
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="flex items-center gap-5">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${isDarkTheme ? 'bg-indigo-900/20' : 'bg-indigo-50'}`} style={{ color: 'var(--accent)' }}>
                  <Calendar size={24} />
                </div>
                <div>
                  <p className={`text-[10px] font-black uppercase ${isDarkTheme ? 'text-gray-400' : 'text-slate-400'}`}>
                    Schedule Date
                  </p>
                  <p className={`text-lg font-black ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
                    {format(new Date(booking.event_date), "EEEE, MMM dd, yyyy")}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-5">
