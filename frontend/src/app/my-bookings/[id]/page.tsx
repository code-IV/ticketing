  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 p-7 flex justify-between items-start z-10">
        <div>
          <h3 className="font-black text-3xl text-white tracking-tighter drop-shadow-lg leading-none">
            {item.game_name}
          </h3>
        ...
        <div className="bg-white/10 backdrop-blur-sm px-4 py-2.5 rounded-2xl border border-white/10 text-right text-white">
          <span className="text-[9px] font-black opacity-50 uppercase block mb-1">
            Rides Left
          </span>
        ...
        <div className="bg-white/10 backdrop-blur-md p-5 rounded-3xl border border-white/10 shadow-xl">
          <div className="flex justify-between items-center text-white/80 text-[11px] font-black uppercase mb-4">
            ...
            <span className="flex items-center gap-1.5 opacity-50">
              <Ticket size={12} /> {totalUsed}/{totalPurchased}
            </span>
          </div>
          <div className="w-full h-1.5 bg-white/20 rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(available / totalPurchased) * 100}%` }}
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 pt-10 px-4">
      <div className="max-w-6xl mx-auto">
          ...
          <AnimatePresence>
            {gameItems.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-slate-500">No game bookings found.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
        ...
        <motion.div
          ...
          className="mt-20 bg-slate-900 rounded-[40px] p-10 flex flex-col md:flex-row items-center gap-8 shadow-2xl"
        >
          ...
          <div className="text-center md:text-left flex-1">
            <h4 className="font-black text-white uppercase text-sm tracking-widest mb-2">
              Getting Scanned
            </h4>
            <p className="text-sm text-slate-400 font-medium">
              Show the Master QR code at any attraction entrance. Operators will
              scan it to deduct a ride from your balance.
            </p>
          </div>
          <button
            onClick={() => setIsQRModalOpen(true)}
            className="px-10 py-5 bg-white text-slate-900 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all"
          >
            Show QR Now
          </button>
        </motion.div>
