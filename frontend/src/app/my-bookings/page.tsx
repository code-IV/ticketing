                ...
                <Card key={item.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      {item.itemType === 'BOOKING' && (
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor((item as any).payment_status)}`}>
                  ...
                  <CardBody className="space-y-3">
                    <p className="text-sm text-gray-600">
                      {item.itemType === 'GAME_TICKET' 
                        ? `${(item as any).quantity} Ticket${(item as any).quantity !== 1 ? 's' : ''} purchased`
