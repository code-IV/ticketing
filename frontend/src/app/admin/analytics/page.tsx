...
import {
  debounce,
  throttle,
} from "@/components/admin/analytics/performanceUtils";
...
export default function AnalyticsPage() {
          ...
          adminService
            ...
            .catch((err) => {
              ...
              return {
                ...
                data: { eventPerformance: [], ticketTypePerformance: [] },
              };
            }),
        ];
  ...
  const aggregatedBookingData = useMemo(() => {
    ...
    if (data.length > 100) {
      ...
      for (let i = 0; i < data.length; i += 7) {
        const weekData = data.slice(i, i + 7);
        const weekBookings = weekData.reduce(
          (sum: number, item: any) => sum + (item.total_bookings || 0),
          0,
        );
        weeklyData.push({
          date: weekData[0]?.date || new Date().toISOString(),
          total_bookings: weekBookings,
          confirmed_bookings: weekData.reduce(
            (sum: number, item: any) => sum + (item.confirmed_bookings || 0),
            0,
          ),
          cancelled_bookings: weekData.reduce(
            (sum: number, item: any) => sum + (item.cancelled_bookings || 0),
            0,
          ),
          pending_bookings: weekData.reduce(
            (sum: number, item: any) => sum + (item.pending_bookings || 0),
            0,
          ),
          total_value: weekData
            .reduce(
              (sum: number, item: any) =>
      ...
      <div>
        ...
        <EventPerformance
          ...
          ticketTypePerformance={
            analyticsData.events?.ticketTypePerformance || []
          }
        />
      </div>
    );
  ...
  const handleDatePresetChange = (presetId: string) => {
    ...
    if (preset && preset.days !== null) {
      ...
      debouncedDateChange((prev: typeof dateRange) => ({
        ...prev,
        preset: presetId,
      }));
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <AnalyticsErrorBoundary>{tabContent}</AnalyticsErrorBoundary>
      </div>
    </div>
  );
}
