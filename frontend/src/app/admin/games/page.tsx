...
import { Plus, Search, Trash2, Activity, Settings, AlertTriangle, Calendar, BarChart3, ArrowUp } from "lucide-react";
...
const GamesManagementPage = () => {
  ...
  const { filteredGames, games, loading, searchQuery, setSearchQuery, statusFilter, setStatusFilter, handleStatusChange, handleDelete, refresh } = useGames();

  const statusConfig = (isDark: boolean) => ({
    OPEN: { bg: isDark ? "bg-green-900/50" : "bg-green-50", text: isDark ? "text-green-400" : "text-green-700", label: "OPEN" },
    ON_MAINTENANCE: { bg: isDark ? "bg-orange-900/50" : "bg-orange-50", text: isDark ? "text-orange-400" : "text-orange-700", label: "ON MAINTENANCE" },
    UPCOMING: { bg: isDark ? "bg-blue-900/50" : "bg-blue-50", text: isDark ? "text-blue-400" : "text-blue-700", label: "COMING SOON" },
    CLOSED: { bg: isDark ? "bg-red-900/50" : "bg-red-50", text: isDark ? "text-red-400" : "text-red-700", label: "CLOSED" },
  });
  ...
  return (
    <div className={`min-h-screen p-8 ${isDarkTheme ? 'bg-[#0A0A0A]' : 'bg-[#F8FAFC]'}`}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        ...
        <button onClick={() => setIsDrawerOpen(true)} className={`flex items-center justify-center gap-2 ${isDarkTheme? "bg-indigo-600" : "bg-gray-800"} text-white px-6 py-3 rounded-2xl font-bold transition-all active:scale-95`}>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        ...
        <div className="flex items-end">
          <Link href="/admin/analitics/games" className={`group flex items-center gap-3 px-6 py-4 rounded-2xl border shadow-lg transition-all ${isDarkTheme ? 'bg-gradient-to-r from-indigo-600 to-purple-600' : 'bg-gradient-to-r from-blue-500 to-purple-500'} text-white font-bold`}>
      ...
      <div className={`p-4 rounded-2xl border shadow-sm mb-6 flex items-center gap-4 ${isDarkTheme ? 'bg-[#0A0A0A] border-gray-700' : 'bg-white border-slate-100'}`}>
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by ride name" className={`w-full pl-12 pr-4 py-3 border-none rounded-xl focus:ring-2 focus:ring-accent2 outline-none font-medium ${isDarkTheme ? 'bg-bg3 text-white' : 'bg-slate-50 text-slate-900'}`} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {filteredGames.map((game) => {
            ...
            return (
              <div key={game.id} onClick={() => router.push(`/admin/games/${game.id}`)} className={`group relative rounded-3xl border shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden cursor-pointer ${isDarkTheme ? 'bg-gradient-to-br from-[#0A0A0A] to-[#1a1a1a] border-gray-700' : 'bg-white border-slate-200'}`}>
                <div className="relative p-8">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-1 group-hover:translate-y-0">
                      <select className="text-[10px] font-bold uppercase px-3 py-1.5 rounded-full border bg-gray-800 text-white border-gray-600" value={game.status} onClick={e => e.stopPropagation()} onChange={e => handleStatusChange(game.id, e.target.value)}>
                        <option value="OPEN">OPEN</option><option value="ON_MAINTENANCE">MAINTENANCE</option><option value="CLOSED">CLOSED</option>
                      </select>
                      <button className="p-2 text-slate-400 hover:text-red-400" onClick={e => { e.stopPropagation(); handleDelete(game.id); }}><Trash2 size={16} /></button>
                    </div>
                  </div>
                  ...
                  <div className={`flex justify-end border-t pt-6 ${isDarkTheme ? 'border-gray-700/50' : 'border-slate-200/50'}`}>
                    <Link href={`/admin/analitics/games/${game.id}`} onClick={e => e.stopPropagation()} className={`group/btn relative px-6 py-3 rounded-2xl font-bold text-sm ${isDarkTheme ? 'bg-gradient-to-r from-indigo-600 to-purple-600' : 'bg-gradient-to-r from-indigo-500 to-purple-500'} text-white`}>
