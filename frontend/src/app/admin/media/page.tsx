...
export default function MediaPage() {
  ...
  useEffect(() => { fetchMedia(); }, [currentPage, filter]);
    ...
    <div className={`min-h-screen ${surface} pt-24 pb-20 px-6 md:px-10`}>
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className={`text-[11px] font-black uppercase tracking-[0.2em] ${muted}`}>Central Assets</span>
            </div>
            <h1 className={`text-5xl md:text-7xl font-black tracking-tighter ${text}`}>
              Media <span className="text-accent italic">Library</span>
            </h1>
          </div>

          <div className={`flex items-center gap-8 px-8 py-5 rounded-4xl border ${border} ${card} shadow-sm`}>
            {[
              { label: 'Total', value: stats.total, icon: <Hash size={14}/> },
              { label: 'Images', value: stats.images, icon: <ImageIcon size={14}/> },
              { label: 'Videos', value: stats.videos, icon: <Video size={14}/> },
            ].map((s, i) => (
              <div key={i} className="flex flex-col items-center">
                <div className="flex items-center gap-1.5 mb-1">
                   <span className="text-accent">{s.icon}</span>
                   <span className={`text-[10px] font-bold uppercase tracking-widest ${muted}`}>{s.label}</span>
                </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`w-10 h-10 rounded-xl font-bold text-sm border ${border} ${card} transition-all ${
                  currentPage === 1 ? 'bg-accent text-black' : 'bg-accent/50 text-white hover:bg-accent2/70 hover:scale-105 active:scale-95'
                }`}
              >
                1
              </button>
              ...
              {totalPages > 1 && (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl font-bold text-sm border ${border} ${card} transition-all ${
                    currentPage === totalPages ? 'bg-accent text-black' : 'bg-accent/50 text-white hover:bg-accent2/70 hover:scale-105 active:scale-95'
                  }`}
                >
                  {totalPages}
                </button>
        ...
        {media.length === 0 && !loading ? (
           <div className="py-20 text-center opacity-40 font-bold uppercase tracking-widest">No items found</div>
        ) : (
