const Discount = {
  async addPromotion(data, client) {
    const sql = `INSERT INTO promotions (name, description, is_active, max_global_usages, total_usages, starts_at, ends_at)
  ...
  async addPromotionRule(promotionId, data, client) {
    const sql = `INSERT INTO promotion_rules (promotion_id, rule_type, rule_data, discount_type, discount_value)
  ...
  async getApplicablePromos(productIds, client) {
    const sql = `SELECT 
    p.*, 
